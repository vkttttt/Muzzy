...
settings.post('/add', async function (req, res) {
    try {
        ...
        if (create.status === true) {
            var key_setting = prefix_setting + dataAdd.key;
            libs.redis.set(key_setting, dataAdd.value, 8400);
...
settings.post('/edit/:id', async function (req, res) {
    try {
        ...
        var update = await adminModel.update(mod_config.collection, { _id: req.params.id }, dataUpdate);
        if (update.status) {
            var key_setting = prefix_setting + dataUpdate.key;
            libs.redis.set(key_setting, dataUpdate.value, 8400);
 ...
 *
 */
settings.post('/import', function (req, res) {
    try {
        ...
        resum.post(req, function (status, filename, original_filename, identifier, numberOfChunks) {
            if ('done' == status && num == total) {
                ...
                filename = helpers.hash.md5(filename.replace(/[^0-9a-zA-Z_-]/gim, ''));
...
settings.get('/list-key-clear', async function (req, res) {
    let key = req.query.key;
    let redis_key = key;
    libs.redis.set(redis_key, '', 1);
...
settings.get('/list-key', async function (req, res) {
    try {
        ...
        let prefix = appConfig.redis.prefix;
        console.log(prefix);
        let list_keys = await libs.redis.list_keys(prefix);
        console.log(list_keys);
        let list = list_keys.filter((keys) => {
            return keys.match(prefix);
        });
        console.table(list);
        dataView.list_keys = list;
...
settings.get('/list-key-value', async function (req, res) {
    let key = req.query.key;

    let list_keys = await libs.redis.get(key);
