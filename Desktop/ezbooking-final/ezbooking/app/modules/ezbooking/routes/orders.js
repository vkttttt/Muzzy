...
orders.get('/', async function(req, res) {
    ...
    try {
        ...
        for (let each in conditions) delete conditions[each]["$options"]
        ...
        if (req.session.admin_userdata.role === 'admin_booking') {
            dataView.notification_lists = await moduleModel.find(mod_config.collection, {'branch_id': req.session.admin_userdata.branch}, "order_id status");
        }
...
//Get add
orders.get('/add', async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req,mod_config);
...
//Post add
orders.post('/add', async function(req, res){
    try {
...
//Get edit
orders.get('/edit/:id', async function (req, res) {
    try {
        //validate
...
//post edit
orders.post('/edit/:id', async function (req, res) {
    try {
        var post_data = req.body;
        req.flash('post_data',post_data);
        console.log('post_data_edit_order',post_data);
        if(post_data.status == 4 || post_data.status == 2){
            post_data.allow_cancel = false;
        }
        ...
        if(update.status === true){
            if(post_data.status == 2 || post_data.status == 4){
                for (var item of update.msg.table_list){ 
                    var table = item.replace(/&quot;/g, '"')
                    table = JSON.parse(table);
                    await moduleModel.update("ezbookingTables",{_id:table._id},{status:'0'});
                  }
            }
...
//GET import
orders.get('/import',async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
        var ignore_fields = '__v update_by createdAt updatedAt';
        dataView.fields = await moduleModel.get_fields(mod_config, ignore_fields);
 ...
 *
 */
orders.post('/import', function(req, res){
    try {
...
//GET export
orders.get('/export', async function(req, res){
    try {
        var fields = await moduleModel.get_fields(mod_config,'__v');
        var field_keys = Object.keys(fields);
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
...
//Export
orders.post('/export', async function(req, res){
    try {
...
//delete
orders.post('/delete', async function (req, res) {
    var post_data = req.body;
...
//Get detail
orders.get('/detail/:_id', async function(req, res){
    try {
        //validate
        var validator = new helpers.validate();
