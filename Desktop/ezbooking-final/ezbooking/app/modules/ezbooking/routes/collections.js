...
//Get add
collections.get('/add', async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req,mod_config);
...
//Post add
collections.post('/add', async function(req, res){
    try {
        ...
        dataAdd.branch_list = [];
        dataAdd.total_branchs = 0;
        var img_url = await helpers.file.writeBase64(
            req.body.thumbnail,
            "public/admin/images/collection"
        );
...
//Get edit
collections.get('/edit/:id', async function (req, res) {
    try {
        //validate
...
//post edit
collections.post('/edit/:id', async function (req, res) {
    try {
        ...
        var oldData = await moduleModel.findOne('ezbookingCollections', { '_id': req.params.id});
        ...
        if (new_thumbnail) {
            ...
            if (oldData.thumbnail && oldData.thumbnail.indexOf("public/admin/images/collection") !== -1) {
        ...
        if(!dataUpdate.branch_list) dataUpdate.branch_list = [];
...
//GET import
collections.get('/import',async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
        var ignore_fields = '__v update_by createdAt updatedAt';
        dataView.fields = await moduleModel.get_fields(mod_config, ignore_fields);
 ...
 *
 */
collections.post('/import', function(req, res){
    try {
...
//GET export
collections.get('/export', async function(req, res){
    try {
        var fields = await moduleModel.get_fields(mod_config,'__v');
        var field_keys = Object.keys(fields);
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
...
//Export
collections.post('/export', async function(req, res){
    try {
...
//delete
collections.post('/delete', async function (req, res) {
    var post_data = req.body;
...
//Get detail
collections.get('/detail/:_id', async function(req, res){
    try {
        //validate
        var validator = new helpers.validate();
