...
//Get add
blogs.get('/add', async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req,mod_config);
...
//Get edit
blogs.get('/edit/:id', async function (req, res) {
    try {
        //validate
...
//post edit
blogs.post('/edit/:id', async function (req, res) {
    try {
        ...
        var oldData = await moduleModel.findOne('ezbookingBlogs', { '_id': req.params.id});
        var new_thumbnail = await helpers.file.writeBase64(
            post_data.new_thumbnail,
            "public/admin/images/blog"
          );
        if (new_thumbnail) {
            //remove old avatar
            //console.log("new_avatar: ", new_avatar);
            if (oldData.thumbnail && oldData.thumbnail.indexOf("public/admin/images/blog") !== -1) {
...
//GET import
blogs.get('/import',async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
        var ignore_fields = '__v update_by createdAt updatedAt';
        dataView.fields = await moduleModel.get_fields(mod_config, ignore_fields);
 ...
 *
 */
blogs.post('/import', function(req, res){
    try {
...
//GET export
blogs.get('/export', async function(req, res){
    try {
        var fields = await moduleModel.get_fields(mod_config,'__v');
        var field_keys = Object.keys(fields);
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
...
//Export
blogs.post('/export', async function(req, res){
    try {
...
//delete
blogs.post('/delete', async function (req, res) {
    var post_data = req.body;
...
//Get detail
blogs.get('/detail/:_id', async function(req, res){
    try {
        //validate
        var validator = new helpers.validate();
