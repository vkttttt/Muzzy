...
var mod_config = {
    module : 'ezbooking',
    resource : 'campaigns',
    collection : 'ezbookingCampaigns',
    route : 'ezbooking/campaigns',
    view : 'campaigns',
    alias : 'campaigns'
};

var campaigns = express.Router();
campaigns.get('/', async function(req, res) {
    ...
    try {
        ...
        if(totals > 0){
            ...
            console.log(req.session.admin_userdata);
            if (branch_id){
...
//Get add
campaigns.get('/add', async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req,mod_config);
...
//Post add
campaigns.post('/add', async function(req, res){
    try {
...
//Get edit
campaigns.get('/edit/:id', async function (req, res) {
    try {
        //validate
...
//post edit
campaigns.post('/edit/:id', async function (req, res) {
    try {
...
//GET import
campaigns.get('/import',async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
        var ignore_fields = '__v update_by createdAt updatedAt';
        dataView.fields = await moduleModel.get_fields(mod_config, ignore_fields);
 ...
 *
 */
campaigns.post('/import', function(req, res){
    try {
...
//GET export
campaigns.get('/export', async function(req, res){
    try {
        var fields = await moduleModel.get_fields(mod_config,'__v');
        var field_keys = Object.keys(fields);
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
...
//Export
campaigns.post('/export', async function(req, res){
    try {
...
//delete
campaigns.post('/delete', async function (req, res) {
    var post_data = req.body;
...
//Get detail
campaigns.get('/detail/:_id', async function(req, res){
    try {
        //validate
        var validator = new helpers.validate();
