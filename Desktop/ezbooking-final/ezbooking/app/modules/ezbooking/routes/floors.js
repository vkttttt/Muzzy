...
var mod_config = {
    module : 'ezbooking',
    resource : 'floors',
    collection : 'ezbookingFloors',
    route : 'ezbooking/floors',
    view : 'floors',
    alias : 'floors'
};

var floors = express.Router();
floors.get('/', async function(req, res) {
...
//Get add
floors.get('/add', async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req,mod_config);
...
//Post add
floors.post('/add', async function(req, res){
    try {
...
//Get edit
floors.get('/edit/:id', async function (req, res) {
    try {
        //validate
...
//post edit
floors.post('/edit/:id', async function (req, res) {
    try {
...
//GET import
floors.get('/import',async function(req, res){
    try {
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
        var ignore_fields = '__v update_by createdAt updatedAt';
        dataView.fields = await moduleModel.get_fields(mod_config, ignore_fields);
 ...
 *
 */
floors.post('/import', function(req, res){
    try {
...
//GET export
floors.get('/export', async function(req, res){
    try {
        var fields = await moduleModel.get_fields(mod_config,'__v');
        var field_keys = Object.keys(fields);
        var dataView = helpers.admin.get_data_view_admin(req, mod_config);
...
//Export
floors.post('/export', async function(req, res){
    try {
...
//delete
floors.post('/delete', async function (req, res) {
    var post_data = req.body;
...
//Get detail
floors.get('/detail/:_id', async function(req, res){
    try {
        //validate
        var validator = new helpers.validate();
...
floors.get('/table_plan/:_id', async function(req, res){
    try {
        //validate
        var validator = new helpers.validate();
        var valid_error = validator.isObjectId(req.params._id,'ID must be ObjectID').hasErrors();
        ...
        var dataView = helpers.admin.get_data_view_admin(req,mod_config);
        var floor = await moduleModel.findOne(mod_config.collection,{'_id':req.params._id});
        var table_plan = await moduleModel.findAll("ezbookingTables",{'floor_id':req.params._id});
        if (floor) {
            dataView.fields = await moduleModel.get_fields(mod_config, '__v');
            dataView.data_edit = dataView.post_data.length > 0 ? dataView.post_data :floor;
            dataView.floor = floor;
            dataView.table_plan = table_plan;
            return res.render('./'+mod_config.module+'/'+mod_config.view+'/table_plan', dataView);
        }else{
            req.flash('msg_error', 'Data null');
            return helpers.base.redirect(res,mod_config.route);
        }
...
floors.post('/table_plan/:_id', async function(req, res){
    try {
        var post_data = req.body;
        req.flash('post_data',post_data);
        //validate
        ...
        var valid_error = validator.isObjectId(post_data._id,'Invalid ID').hasErrors();
        if(valid_error){
            console.log('msg_error',valid_error[0]);
            return helpers.base.redirect(res,mod_config.route);
        }

        if(!post_data._id || post_data._id != req.params._id){
            console.log('valid_errors','Invalid ID');
            return helpers.base.redirect(res,mod_config.route);
        }
        var floor_data = await moduleModel.filterData(mod_config.collection,post_data,'_id');
        floor_data.update_by = helpers.admin.get_update_by(req);
        var update = await moduleModel.update(mod_config.collection, {'_id': req.params._id}, floor_data,true);
        
        var table_list = post_data.table_list;
        var status_list = post_data.status_list;
        var floor = await moduleModel.findOne(mod_config.collection,{'_id': req.params._id});
        var status = 0;
        for(var i = 0; i < table_list.length; i++){
            if(status_list[i]) status = status_list[i];
            var table = {
                // restaurant_id: floor_data.restaurant_id,
                floor_id: post_data._id,
                branch_id: post_data.branch_id,
                slot: table_list[i],
                position_x: Math.floor(i/parseInt(floor.width)),
                position_y: i % parseInt(floor.width),
                status: status,
                update_by: floor_data.update_by,
            }
            var where = {
                floor_id: post_data._id,
            ...
            var old_table = await moduleModel.findOne('ezbookingTables',where);
            if(old_table){
                await moduleModel.update('ezbookingTables', {_id: old_table._id},table);
            }
            else{
                await moduleModel.create('ezbookingTables',table);
            }
        }
        if(update.status === true){
            req.flash('msg_success','Edit success.');
